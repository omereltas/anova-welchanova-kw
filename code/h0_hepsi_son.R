# rerun_negbin_het_fixed.R
# Negatif binom heterojen h??crelerini var_type dahil seed ile yeniden ??al????t??r??r

library(dplyr)

# ========= AYARLAR =========
OUT_CSV   <- "negbin_het_rerun_fixed.csv"
CKPT_DIR  <- "rerun_ckpt_fixed"
...
# ========= D??ZELT??LM???? SEED FONKS??YONU =========
# var_type art??k seed hesaplamas??na dahil ediliyor
make_seed <- function(n, k, balance, dist, var_type) {
  SEED_BASE +
    ...
    nchar(balance)       *   1000 +
    nchar(dist)          *    100 +
    nchar(var_type)      *     10   # <-- EKLEND??
}

# ========= ANA S??M??LASYON FONKS??YONU =========
run_one_cell <- function(n, k, balance, dist, var_type) {
  ...
  while (acc < TARGET_ACCEPTS && attempts < MAX_ATTEMPTS) {
    ...
    if (eta2 >= eta2_min && eta2 <= eta2_max) {
      ...
      if (acc %% SAVE_EVERY == 0L) {
        ...
        message("Checkpoint kaydedildi: ", cell_id,
  ...
  if (!complete) {
    warning("TAMAMLANAMADI: ", cell_id,
            " | Kabul edilen: ", acc, " / ", TARGET_ACCEPTS)
  }
...
# ========= ??ALI??TIRILACAK H??CRELER =========
# Sadece negatif binom heterojen ??? t??m n/k/balance kombinasyonlar??
target_cells <- expand.grid(
  n        = c(5, 6, 8, 10),
  k        = c(3, 5, 8),
  balance  = c("balanced", "unbalanced"),
  dist     = "negbin",
  var_type = "heterogeneous",
...
message("\n--- HOMOGENEOUS ile KARSILASTIRMA KONTROLU ---")
message("Heterojen guc degerleri (ilk 6 satir):")
print(head(results[, c("n","k","balance","var_type",
